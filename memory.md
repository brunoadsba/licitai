...
- **Branch ativa (24/09/2026)**: `fix/quarentena-tcu` (Fase 0B). `main` em `ddcbe58`. CI permanece desabilitado.
- **Fase 0B — quarentena TCU (24/09/2026)**: fontes sem URL oficial (`Súmula 247/TCU`, `Súmula 272/TCU`, `Acórdão 1214/2013-TCU-Plenário`) saem da busca padrão, de `legal_basis` e do parecer; dados preservados com `version=quarantine-0B`. Lista em [docs/ops/quarentena-tcu.md](docs/ops/quarentena-tcu.md) e `backend/data/tcu/pending-confirmation.json`. Validado no Postgres piloto (retrieve sem TCU; leis/RILC ok). **Próxima fase técnica: 0C (token com Postgres + extensão SEI).**
...
| **0C** Auth operacional + extensão SEI | **Próxima** | `API_TOKEN` obrigatório com Postgres; docs fora de dev; escape HTML da extensão |
...
2. **Próxima sessão técnica:** Fase **0C** — token obrigatório com Postgres + escape da extensão SEI ([plano-tecnico-ajustado.md](docs/ops/plano-tecnico-ajustado.md)).
...
> **Fase 0B quarentena TCU (24/09/2026, `fix/quarentena-tcu`):** súmulas 247/272 e Acórdão 1214/2013 fora da busca/`legal_basis`/parecer; `version=quarantine-0B` no Postgres piloto; retrieve validado (só lei/RILC). **Próximo código: Fase 0C.**