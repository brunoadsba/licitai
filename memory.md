...
- **Copiloto LicitAI (chat consultivo, 06/08/2026)**:
  ...
  - Frontend: `hooks/useChat.ts`, `components/chat/{ChatCopilot,ChatPanel,ChatMessage,AssistantAnswer,ChatInput,CitationList}.tsx` — FAB **Perguntar** abre diálogo **centralizado** (quase tela cheia); integrado em `analysis/[id]/page.tsx`.
...
- **Confiabilidade LicitAI — plano mestre implementado (08/09/2026, branch `feat/confiabilidade-master`)**:
  ...
  - **Pendência manual (Bruno)**: rotacionar chaves LLM/`POSTGRES_PASSWORD`/`API_TOKEN` — **descartada em 26/09/2026** (chaves provisórias do MVP, decisão do Bruno); smoke Compose com worker; backup drill.
...
- `docs/ops/`: Deploy imutável, restore drill, SLOs, piloto, gate 14d, cron. Plano técnico pós-auditoria (ondas 0A–0C e fases 1–9): [plano-tecnico-ajustado.md](docs/ops/plano-tecnico-ajustado.md). Plano 30/60/90 (oportunidades E2E 25/09): [plano-30-60-90.md](docs/ops/plano-30-60-90.md). Auditoria E2E via LLM: [e2e-full.md](docs/ops/e2e-full.md) + script `scripts/contexto-auditoria.sh` + briefing [`.cursor/contexts/auditoria-e2e-falhas.md`](.cursor/contexts/auditoria-e2e-falhas.md).
...
- **Ollama local qwen3:8b (26/09/2026)**: `ollama pull qwen3:8b` (~5,2 GB) no host WSL; `docker-compose.yml` com `extra_hosts host.docker.internal:host-gateway` no backend+worker; `ollama.service` com override permanente `OLLAMA_HOST=0.0.0.0:11434` (sudo NOPASSWD escopado em `/etc/sudoers.d/ollama-manage`: daemon-reload + restart/status do serviço + criar o override); `.env` com `OLLAMA_BASE_URL=http://host.docker.internal:11434` + `OLLAMA_MODEL=qwen3:8b`. Validado: fail-closed sigiloso + geração real via `OllamaProvider` dentro do container (44s aquecido; ~80s frio — CPU-only ~10 tok/s com thinking, análise econômica completa levará horas). Fallback sem sudo preservado em `backend/scripts/ollama_proxy.py` (0.0.0.0:11435 → 127.0.0.1:11434, com fix de timeout herdado do `create_connection`; serviço user desativado).
...
- **Fase 5 — recuperação (24/09/2026, `f81ca21` + sobras)**: FTS AND/OR + rerank em `rag_candidates`. Piloto r@5 **0.929**. `embedding_vector` preenchido via `backfill_embedding_vector.py` (sem HNSW). Doc: [docs/ops/recuperacao-fase5.md](docs/ops/recuperacao-fase5.md).
...
- **Fase 2 — avaliação real e linha de base (24/09/2026)**: conjunto `backend/eval/cases.json`; runner `scripts/eval_corpus_real.py`; baselines em `eval/baseline.ci.json` (r@5 1.0) e `eval/baseline.piloto.json` (r@5 **0.929**). Doc: [docs/ops/eval-fase2.md](docs/ops/eval-fase2.md). Visto jurídico pendente. CI GitHub desligado.
...
| 12 | ~~**Copiloto UX**~~ | Feito (25/09, branch) | Chat centralizado (quase tela cheia). Resposta em Resposta / O que fazer agora / Onde está no TR; sem UUID nem `estrutural:failed`. Branch `feat/copiloto-ux` |
...
2. Código agent-implementável das Fases **0A–8 e sobras** + UX simples/Fontes + omissão Art. 6º está em **`main`**. Copiloto tela cheia + resposta-guia está em **`feat/copiloto-ux`** (merge só se você pedir). Não reabrir 5–8 sem regressão de r@5. Não tirar TCU da quarentena de RAG sem visto. UI Docker exige `./scripts/up.sh --build`. Fase 9 / CI / K8s / fine-tune / OIDC: fora de escopo.
...
> **Gate autônomo (15/09/2026, merge `9bb3ed8` em `main`)**: plano `.omo/plans/gate-autonomo-2026-09-15.md`. Fase 0: 4 healthy + 265 passed. Fase 1: re-upload `09-ti-pabx-nuvem` (doc `626cabf0…`, 257 itens) + análise economic (`20de1bb8…`, ~13 min, `completed_with_errors` esperado, 12/257 itens, nota 9.0/risco crítico, **art6 100%**, 9 correções, SEI/HTML/DOCX 200) — relatório `docs/ops/gate-reanalise-09-ti-2026-09-15.md`. Fase 2: heurística 5 TRs média **76%** (igual baseline 14/09), piloto 71% 2/12 — `docs/ops/quinzena-2026-09-15-parcial.md`. Fase 3: 4 PDFs da Emergência com 1 e-mail cada + `backend/scripts/anonymize_emergencia.py --dry-run` ok; `fixtures/` intocado. Fase 4: `promote_feedback --dry-run` ok, sem stubs reais. Fase 5: branch marcador `spike/art6-coverage-15-09` (sem merge até 28/09) + relatório `docs/ops/spike-art6-2026-09-15.md` (golden 13 passed); gaps são curadoria dos TRs, não do validador. **Pós-gate (15/09, com ok do Bruno): `anonymize_emergencia.py --apply` em 2 rodadas — e-mails (5× `[email]`) + telefones BR (9× `(47) 3341-8065` COAMB e `(71) 3320-1310` rodapé; script estendido com regex `PHONE` + 5º alvo `piloto-unico/06`); verificação: 0 `@`, 0 fones restantes nos 5 PDFs; backup `backups/anonymize-emergencia-2026-09-15/` intacto.**
> **Pendências (15/09)**: Solange enviar TRs, colar SEI real, secrets; Emergência anonimizada (e-mails + telefones); ML bloqueado até dataset.
...
> **RAG moderno R0–R4 (18/09/2026, branch `feat/rag-moderno-18-09`, plano `.omo/plans/rag-moderno-2026-09-18.md`):** harness HARD `test_rag_recall.py` (44 chunks, 16 queries, 6 categorias) começou falhando (0.56) e expôs 3 defeitos reais — RRF assimétrico→clássico, 20→50 candidatos, FTS sem stopwords/stemming→stopwords PT + prefixo `*` + overlap por prefixo; resultado **Recall@5 1.0, MRR 1.0**; `rerank.py` (heurístico + `llm_rerank` fail-open opt-in, default heuristic), `retrieve()` 50→top-k, fiação engine com trava sigiloso/cloud, `claim_support_rate()` no evidence JSON, chunks contextuais no `ingest_embeddings.py` + `embedding_dim` (reingest piloto pendente). Robustez: `test_rag_robustness.py` (determinismo, fallback, cache, hostis). **297 passed · LSP 0.** Relatório em `backend/rag_eval_report.md`.
...
> **Copiloto UX (25/09/2026, `feat/copiloto-ux`)**: diálogo centralizado `min(96vw, 1100px)` × `min(92dvh, 860px)`. Prompt guia (Resposta / O que fazer agora / Onde está no TR). Sanitiza UUID, `source_id` e `estrutural:failed`. Fontes sem rodapé de IDs do parecer e sem correção operacional. UI: `AssistantAnswer` + citações sem UUID.
> **Congelamento 300+ LOC (25/09/2026, Onda 2)**: próxima feature em `rag/loader.py` (339), `analyzer/evidence_gate.py` (324), `rag/semantic.py` (323), `analyzer/analysis_persistence.py` (315) ou `api/documents.py` (311) **extrai módulo** — sem refactor especulativo agora.