# Guia do usuário — LicitAI

Para o elaborador de Termo de Referência (TR) na CODEBA.

Na aplicação: **Mais ferramentas → Guia do usuário** ou `/guia`.

**Regra:** só vai para o SEI o que você **aprovou** ou **ajustou**.

---

## Fluxo (4 passos)

1. **Enviar** o TR (PDF ou DOCX, até 50 MB)
2. **Aguardar** a análise
3. **Revisar** (Aprovar / Rejeitar / Ajustar)
4. **Copiar pacote SEI** e colar no processo

---

## Telas

### Painel
- Lista dos TRs e o status de cada um (ex.: Pronto para análise, Concluído).
- **Enviar TR** — novo arquivo.
- Por linha: **Analisar**, **Ver resultado** ou **Acompanhar**.
- Se houver pendências: **Continuar revisão**.

### Enviar TR
1. Arraste ou escolha o arquivo.
2. Clique em **Enviar e Analisar**.
3. Acompanhe o progresso na tela.

**Opções avançadas** (recolhidas): análise completa, proposta, comparação de versões. Abra só se precisar.

### Análise
- Fila **Revisar agora** = risco alto/crítico + Art. 6º. Use **Ver todas** para o restante.
- Painel **Art. 6º** mostra a cobertura estrutural do TR (meta sugerida ≥ 90%).
- Em cada sugestão: **Aprovar**, **Rejeitar** ou **Ajustar**.
- Com pelo menos uma aprovada/ajustada: **Copiar pacote SEI**.
- **Exportar:** HTML, DOCX ou pacote `.md`.
- **Mais:** relatório, histórico, reanalisar.
- **Perguntar:** dúvidas sobre o documento.

### Relatório
Resumo e **Exportar PDF** (impressão do navegador). O pacote SEI fica na Análise.

---

## Colar no SEI

1. Na Análise, **Copiar pacote SEI**.
2. Cole na minuta/processo no SEI.
3. Confira o texto antes de concluir.

---

## Outros

| Item | Onde |
|------|------|
| Tema claro/escuro | Interruptor no cabeçalho |
| Gerar TR, Comparações, Versões, Moldes | Mais ferramentas (opcional) |

---

## Problemas comuns

| Situação | O que fazer |
|----------|-------------|
| Pacote SEI desabilitado | Aprove ou ajuste ao menos uma correção |
| Arquivo rejeitado | Use PDF ou DOCX, até 50 MB |
| Painel / Comparações / Moldes vazios | Normal no início: envie um TR no Painel; moldes e fornecedores só se for usar auditoria |
| Sistema indisponível | Avise o suporte / equipe do piloto |

---

*Acesso e ambiente: suporte CODEBA / equipe do piloto LicitAI.*
