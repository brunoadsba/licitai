...
- Baseline do piloto (599 chunks): `backend/eval/baseline.piloto.json` (Recall@5 **0.929** após FTS AND/OR + rerank).
- `legal_review` no conjunto: **pendente** de visto jurídico.

A busca textual do Postgres usa FTS (`tsvector` português + GIN), AND de conteúdo com OR de complemento e rerank. A semente continua em 1.0; o corpus cheio foi de 0.214 (ILIKE) → 0.357 (FTS simples) → **0.929**. O visto jurídico do conjunto permanece pendente.
